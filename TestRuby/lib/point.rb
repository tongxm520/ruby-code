...
#定义了each iterator的类可以使用Enumerable module中定义的方法,例如all?方法。必须用include加载
class Point
  attr_reader :x,:y
  def initialize(x,y)
    @x,@y=x,y
...
#p4=p1+p3+p2
#p5=-p4
#puts p5    #返回类中最后一个表达式的值，这里是最后一个方法的返回值
#puts p5["x"]
#puts p5[:x]
#puts p5[0]
#puts p5[-2]
...
#puts p5[5]
p1.each {|x| puts x}
puts p2.all? {|x| x==0}          #定义了each iterator的类可以使用Enumerable module中定义的方法,例如all?方法。必须用include加载
